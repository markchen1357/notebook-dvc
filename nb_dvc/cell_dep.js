...
const code = fs.readFileSync('script.py', 'utf8');
...
for (let flow of flows.items) {
    let def_line = flow.fromNode.location.first_line
    let use_line = flow.toNode.location.first_line
    let cell1 = getCell(def_line);
    let cell2 = getCell(use_line);
    if (!sameCell(def_line, use_line) && cell1 != 'cell0' && cell2 != 'cell0'){
        
        var name = []
     
        let def = analyzer.getDefs(flow.fromNode, new py.RefSet()).items.map(x=>x.name)
        
        let use = analyzer.getUses(flow.toNode).items.map(x=>x.name)
        def = def.filter(x=>use.includes(x))
    
        def = def.map(x=>[cell1,cell2,x])
